...
- **桌面端外壳中的内嵌 WebView**——Desktop 产品现在独立拥有这项能力；参见 [Desktop 应用内浏览器与产物预览](2026-09-04-desktop-in-app-browser-and-artifact-preview.zh.md)。web 产出文件交互面仍使用 Host 打开器，因为它可能并不运行在 Host 机器上。
