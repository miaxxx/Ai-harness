...
The Desktop ACP composition mounts `dsh-compaction-tool-result-pruner` beside the token meter and compaction backend. A single oversized MCP or other tool result is therefore reduced without a model call before compaction attempts a summary. The original result remains in the append-only log, while the replacement surface lets both the current turn and a later user follow-up recover from the same session.
