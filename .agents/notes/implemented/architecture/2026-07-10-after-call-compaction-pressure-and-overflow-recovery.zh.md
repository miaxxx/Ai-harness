...
Desktop ACP 装配会在 token meter 与压缩后端旁挂载 `dsh-compaction-tool-result-pruner`。因此，单个过大的 MCP 或其他工具结果会先在不调用模型的情况下缩减，随后压缩才会尝试生成摘要。原始结果仍保留在只追加日志中，而替换后的表层使当前 turn 与用户之后在同一会话中发送的消息都能恢复。
...
单元测试覆盖最终适配器规范化边界（包括提供方专用 400 错误所包装的 token 数量措辞）、已关闭 turn 的重试编号与重置、取消与 dispose、step 边界顺序、已路由信封压力、压力门控剪枝、剪枝独立解除压力、从已剪枝输入生成摘要、平衡溢出缩减、后续失败前已落盘的剪枝进展、generation 证明、上限、委托与辅助调用路由。真实循环测试覆盖抛出式和带内溢出在经剪枝或摘要压缩后重建重试请求的过程。Desktop 分发 smoke 会先验证内嵌 ACP 配置与运行时都包含剪枝器，再启动已迁移位置的应用。
