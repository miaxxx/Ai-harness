...
The Provider supports native `app` and `desktop` targets. App observations are semantic/accessibility-first and return observation-scoped element ids. Supported mutations include semantic element actions plus bounded coordinate click/drag and desktop scrolling; every mutation is followed by a fresh observation. Global Desktop input does not require a discoverable frontmost Accessibility process. When window-scoped pixels are unavailable, the Provider returns `WINDOW_UNAVAILABLE` instead of mislabeling a full-desktop screenshot as an application image.
