...
| [`computer/computer`](../packages/computer/computer/README.zh.md) | 本机桌面/浏览器检查与已批准输入的 Provider 选择 | `ctx.computer` |
...
| 添加受控浏览器或桌面自动化 | 注册 `ctx.computer` Provider，并通过具备授权意识的工具消费它 |
