...
function validateWebSearchUpdate(value: unknown, existing: StoredWebSearchSettings | undefined): StoredWebSearchSettings {
  ...
  if (!safeStorage.isEncryptionAvailable()) {
    throw new Error('macOS secure storage is unavailable; the You.com API Key was not saved')
  }
  return {
...
function isBrowserPreviewable(mediaType: string): boolean {
...
function previewWebUrl(value: unknown): string {
  const url = new URL(nonEmptyString(value, 'url'))
...
function promptParts(value: unknown): DesktopPromptPart[] {
  if (!Array.isArray(value) || value.length === 0) throw new Error('prompt must contain text or attachments')
  return value.map((part): DesktopPromptPart => {
    if (typeof part !== 'object' || part === null) throw new Error('prompt contains an invalid part')
    const candidate = part as Partial<DesktopPromptPart>
    if (candidate.type === 'text' && typeof candidate.text === 'string') return { type: 'text', text: candidate.text }
    if (candidate.type === 'attachment' && typeof candidate.attachmentId === 'string' && candidate.attachmentId !== '') {
      return {
        type: 'attachment',
        attachmentId: candidate.attachmentId,
        name: typeof candidate.name === 'string' && candidate.name !== '' ? candidate.name : '附件',
      }
    }
    throw new Error('prompt contains an invalid part')
  })
}

function attachmentPaste(value: unknown): DesktopAttachmentPaste {
  if (typeof value !== 'object' || value === null) throw new Error('attachment paste must be an object')
  const row = value as Partial<DesktopAttachmentPaste>
  const selection = row.selection as DesktopAttachmentPaste['selection'] | null | undefined
  if (!isStringArray(row.paths) || typeof row.imageWithoutPath !== 'boolean'
    ...
    throw new Error('attachment paste has invalid fields')
  }
  const paths = row.paths.map((path) => {
    if (!isAbsolute(path)) throw new Error('clipboard attachment paths must be absolute')
    return resolve(path)
  })
  return { paths, imageWithoutPath: row.imageWithoutPath, selection }
}

async function clipboardPayload(readImage: boolean): Promise<{ paths: string[]; image?: Uint8Array }> {
  const items = await clipboard.read()
  const values: string[] = []
  let image: Uint8Array | undefined
  for (const item of items) {
    for (const type of item.types) {
      const fileUrl = type === 'text/uri-list' || type.includes('public.file-url')
      const png = readImage && image === undefined && type === 'image/png'
      if (!fileUrl && !png) continue
      const payload = await item.getType(type)
      if ('url' in payload) continue
      if (fileUrl) values.push(await payload.text())
      else image = new Uint8Array(await payload.arrayBuffer())
    }
  }
  const paths: string[] = []
  for (const value of values.flatMap(value => value.split(/\r?\n/))) {
    if (!value.startsWith('file:')) continue
    try { paths.push(fileURLToPath(value)) } catch { /* Ignore malformed non-file clipboard entries. */ }
  }
  return { paths, ...(image === undefined ? {} : { image }) }
}
...
class AcpRuntimeSupervisor {
  ...
  async prompt(sessionId: string, parts: readonly DesktopPromptPart[]): Promise<DesktopPromptResult> {
    ...
    for (const part of parts) {
      ...
      prompt.push(...await desktopContent().promptBlocks(sessionId, [part.attachmentId]))
    ...
    desktopContent().consumeAttachments(sessionId, [...new Set(attachmentIds)])
...
function installIpc(): void {
  ...
  ipcMain.handle('dsh:session-prompt', async (event, rawSessionId: unknown, rawPrompt: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    return supervisor.prompt(
      nonEmptyString(rawSessionId, 'sessionId'),
      promptParts(rawPrompt),
    )
  })
  ...
  ipcMain.handle('dsh:preview-prepare', async (event, value: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    const path = workspacePath(value)
    const info = await stat(path)
    if (info.isDirectory()) return { kind: 'directory' as const, path }
    if (!info.isFile()) throw new Error('Preview target must be a file or directory')
    const mediaType = desktopMediaType(path)
    return {
      kind: 'file' as const,
      path,
      ...
      title: basename(path),
      mediaType,
      previewable: isBrowserPreviewable(mediaType),
    }
  })
  ipcMain.handle('dsh:url-open-external', async (event, value: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    await shell.openExternal(previewWebUrl(value))
  })
  ...
  ipcMain.handle('dsh:attachment-paste', async (event, rawSessionId: unknown, rawCwd: unknown, rawPaste: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    const sessionId = nonEmptyString(rawSessionId, 'sessionId')
    const cwd = workspacePath(rawCwd)
    const paste = attachmentPaste(rawPaste)
    const payload = await clipboardPayload(paste.imageWithoutPath)
    const paths = [...new Set([...paste.paths, ...payload.paths])]
    const added = await desktopContent().referenceAttachments(sessionId, cwd, paths)
    if (payload.image !== undefined) {
      added.push(await desktopContent().stageClipboardImage(sessionId, cwd, payload.image))
    }
    return added
  })
  ipcMain.handle('dsh:attachment-reference', async (event, rawSessionId: unknown, rawCwd: unknown, rawPaths: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    if (!isStringArray(rawPaths) || rawPaths.length === 0) throw new Error('attachment paths must be a non-empty string array')
    const paths = rawPaths.map((path) => {
      if (!isAbsolute(path)) throw new Error('attachment paths must be absolute')
      return resolve(path)
    })
    return desktopContent().referenceAttachments(
      nonEmptyString(rawSessionId, 'sessionId'), workspacePath(rawCwd), paths,
    )
  })
  ...
  ipcMain.handle('dsh:model-settings-save', async (event, value: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    const existing = await readStoredModelSettings()
    const update = validateModelUpdate(value, existing)
    const sameEndpoint = existing !== undefined
      && update.stored.baseURL === existing.baseURL
      && update.stored.model === existing.model
      && update.stored.protocol === existing.protocol
      && update.stored.encryptedApiKey === existing.encryptedApiKey
      && existing.capabilities.verified
    const capabilities = sameEndpoint
      ? existing.capabilities
      : await probeDesktopModel({
        baseURL: update.stored.baseURL,
        model: update.stored.model,
        protocol: update.stored.protocol,
        apiKey: update.apiKey,
      })
    const stored: StoredModelSettings = { ...update.stored, capabilities }
    await writeStoredModelSettings(stored)
    await supervisor.restart()
    return publicModelSettings(stored)
  })
  ipcMain.handle('dsh:web-search-settings', async (event) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    return publicWebSearchSettings(await readStoredWebSearchSettings())
  })
  ipcMain.handle('dsh:web-search-settings-save', async (event, value: unknown) => {
    if (!trustedSender(event)) throw new Error('desktop IPC rejected an untrusted sender')
    const stored = validateWebSearchUpdate(value, await readStoredWebSearchSettings())
    await writeStoredWebSearchSettings(stored)
    await supervisor.restart()
    return publicWebSearchSettings(stored)
  })
    ...
    try {
      ...
      return new Response(body, {
        headers: {
          ...
          'content-security-policy': "default-src 'none'; script-src 'self'; style-src 'self'; font-src 'self' data:; img-src 'self' data: blob:; frame-src http: https: file: about:; connect-src 'none'; base-uri 'none'; form-action 'none'",
        },
      })
    } catch {
...
async function createWindow(): Promise<BrowserWindow> {
  ...
  const routePreviewUrl = (url: string): void => {
    try {
      window.webContents.send('dsh:preview-request', previewWebUrl(url))
    } catch {
      // Non-web destinations stay blocked by the product Renderer.
    }
  }
  ...
  window.webContents.on('will-attach-webview', (event, webPreferences, params) => {
    ...
    try {
      ...
      allowed = protocol === 'http:' || protocol === 'https:' || protocol === 'file:' || params.src === 'about:blank'
    } catch {
      allowed = false
    }
    if (!allowed) {
    ...
    contents.setWindowOpenHandler(({ url }) => {
      try {
        ...
        if (parsed.protocol === 'http:' || parsed.protocol === 'https:' || parsed.protocol === 'file:') {
          void contents.loadURL(parsed.href)
        }
      } catch {
        // An invalid or executable destination remains blocked.
      }
      return { action: 'deny' }
    })
