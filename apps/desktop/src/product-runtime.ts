...
import type {
  ContentBlock, RpcResult, SessionId, WorkspaceId, WorkspaceView,
...
const WORKSPACES_KEY = 'dsh.desktop.workspaces.v1'

interface DesktopProjectionFace extends ProjectionsFace {
...
function toolOutput(update: unknown): ContentBlock[] {
  ...
  for (const item of content) {
    ...
    blocks.push(...textContent(nested))
  }
  return blocks
}
...
function emptyProjectionFace(): DesktopProjectionFace {
...
function buildEmptySnapshot(sessionId: SessionId, blank: boolean): ConversationSnapshot {
...
class DesktopSession implements SessionFace {
  readonly projections: DesktopProjectionFace = emptyProjectionFace()
  ...
  constructor(
    ...
    this.store = createSnapshotStore(buildEmptySnapshot(sessionId, options.blank))
  }
  ...
  summary(title?: string): SessionSummary {
    return {
      ...
      displayTitle: title ?? this.sessionId,
      cwd: this.cwd,
  ...
  accept(notification: DesktopSessionNotification): void {
    ...
    switch (update.sessionUpdate) {
      ...
      case 'tool_call_update': {
        ...
        if (entry === undefined) {
          entry = {
            kind: 'tool', id, title: id, rawInput: '{}', status: update.status, output: [],
          }
          this.tools.set(id, entry)
          this.entries.push(entry)
        }
  ...
  private syntheticEvents(): SessionEvent[] {
    ...
    const append = (event: Omit<SessionEvent, 'seq' | 'time'>): void => {
      events.push({ ...event, seq: seq++, time: now + seq } as SessionEvent)
    }
    const openTurn = (): void => {
      if (turnOpen) return
      turn += 1
  ...
  private publish(): void {
    const events = this.syntheticEvents()
    this.assembler.replaceWindow(events.map(event => ({ event })), false)
    ...
    this.store.update((draft) => {
      ...
      draft.composerPhase = this.blank
        ? (this.promptAttempted ? 'engaging' : 'blank')
        : 'active'
  ...
  async prompt(content: Array<{ type: string; text?: string }>, _mode: 'queue' | 'steer'): Promise<RpcResult<{ accepted: true }>> {
  ...
  readAttachment(): Promise<RpcResult<never>> {
    return Promise.resolve(rpcFailure('Historical attachment loading is not exposed by the standard ACP bridge yet.'))
  }

  updateQueue(): Promise<RpcResult<{ accepted: true }>> {
    return Promise.resolve(rpcFailure('Queue editing requires the DSH ACP product extension and is not available yet.'))
  }
  ...
  rename(): Promise<RpcResult<{ title: string; seq: number }>> {
    return Promise.resolve(rpcFailure('Session rename requires the DSH ACP product extension and is not available yet.'))
  }
  ...
  command(): Promise<RemoteResult<{ matched: boolean }>> {
...
class DesktopSessions implements ISessions {
  ...
  private readonly records = new Map<SessionId, DesktopSessionRecord>()
  private readonly channel: SessionProvideChannel
  private titles = new Map<SessionId, string | undefined>()
    ...
    this.channel = new SessionProvideChannel({
      rebuildBundles: () => {
        for (const [id, record] of this.records) {
          if (record.provideInfo !== undefined) record.provideInfo = this.channel.materializeInfo(this.binding(id) as SessionBinding)
        }
      },
  ...
  async refresh(paths: readonly string[]): Promise<void> {
    ...
    this.list.update((draft) => {
      draft.ids = [...found.keys()]
        .sort((left, right) => (draft.byId[right]?.updatedAt ?? 0) - (draft.byId[left]?.updatedAt ?? 0))
  ...
  async create(cwd: string): Promise<SessionId> {
    ...
    const row: DesktopSessionSummary = { sessionId: id, cwd }
    this.adopt(row, { loaded: true, blank: true })
    this.list.update((draft) => {
  ...
  accept(frame: Extract<DesktopRendererFrame, { type: 'session-update' }>): void {
    ...
    record.session.accept(frame.notification)
  }
  ...
  private adopt(row: DesktopSessionSummary, options?: { loaded?: boolean; blank?: boolean }): DesktopSessionRecord {
  ...
  private provideInfo(id: SessionId): SessionProvideInfo | undefined {
    ...
    record.provideInfo ??= this.channel.materializeInfo(this.binding(id) as SessionBinding)
  ...
  scope(id: SessionId): AgentContext | undefined {
    ...
    if (record.ctx === undefined) {
      const scope = createScope(this.rootCtx, id)
      record.ctx = scope.ctx
      record.fiber = scope.fiber
    }
    return record.ctx
  }
  ...
  openSubagent(): void {}
  subagentAddress(): undefined { return undefined }
  setSubagentCatalogOpen(): void {}
  refreshSubagents(): Promise<void> { return Promise.resolve() }
  noteAgentPreset(): void {}

  search(): ReturnType<ISessions['search']> {
    return Promise.resolve({ ok: true, value: { items: [], hasMore: false } })
  }

  fork(): Promise<SessionId> {
    return Promise.reject(new Error('Session fork requires the DSH ACP product extension and is not available yet.'))
  }
}
    ...
    if (raw !== null) {
      ...
      if (Array.isArray(parsed)) {
        const rows = parsed.flatMap((value): StoredWorkspace[] => {
          if (typeof value !== 'object' || value === null) return []
          const row = value as { workspaceId?: unknown; path?: unknown; title?: unknown }
          if (typeof row.workspaceId !== 'string' || typeof row.path !== 'string' || typeof row.title !== 'string') return []
          return [{ workspaceId: row.workspaceId as WorkspaceId, path: row.path, title: row.title }]
        })
        if (rows.length > 0) return rows
      }
    }
  } catch {}
  return [{ workspaceId: initialPath as WorkspaceId, path: initialPath, title: basename(initialPath) }]
}

class DesktopWorkspaces implements IWorkspaces {
  ...
  private project(): void {
    ...
    const items: WorkspaceView[] = this.rows.map(row => ({
      ...
      title: row.title,
      path: row.path,
      sessionIds: sessions.ids.filter(id => sessions.byId[id]?.cwd === row.path),
    }))
    this.list.update((draft) => {
      draft.items = items
      draft.state = 'idle'
  ...
  async create(input: { path: string }): Promise<WorkspaceView> {
    const existing = this.rows.find(row => row.path === input.path)
    if (existing === undefined) {
      this.rows.push({ workspaceId: input.path as WorkspaceId, path: input.path, title: basename(input.path) })
      this.persist()
      await this.sessions.refresh(this.rows.map(row => row.path))
    }
    this.project()
    return this.list.getSnapshot().items.find(item => item.path === input.path) as WorkspaceView
  }
  ...
  async listDirectory(path?: string): Promise<DesktopDirectoryListing> {
  ...
  async rename(workspaceId: WorkspaceId, title: string): Promise<WorkspaceView> {
    ...
    this.persist()
    this.project()
    return this.list.getSnapshot().items.find(item => item.workspaceId === workspaceId) as WorkspaceView
  }
  ...
  async insertSessionBefore(workspaceId: WorkspaceId, sessionId: SessionId): Promise<WorkspaceView> {
    const view = this.list.getSnapshot().items.find(item => item.workspaceId === workspaceId)
    if (view === undefined) throw new Error(`desktop workspaces: unknown workspace ${workspaceId}`)
    return { ...view, sessionIds: [...new Set([...view.sessionIds, sessionId])] }
  }

  async archiveSession(sessionId: SessionId): Promise<void> {
    this.list.update((draft) => {
      if (!draft.archivedSessionIds.includes(sessionId)) draft.archivedSessionIds = [...draft.archivedSessionIds, sessionId]
    })
...
class LocalSettingsScope<T extends Record<string, unknown>> implements SettingsScope<T> {
  ...
  getSnapshot(): SettingsScopeSnapshot<T> {
    return {
      status: 'ready', value: this.value, base: undefined, user: this.value,
      revision: this.revision, writable: true, mode: 'memory',
    }
  }
...
function remoteStub(): object {
  ...
  return new Proxy(namespace, {
    get: (target, property, receiver) => Reflect.get(target, property, receiver) ?? namespace,
  })
}
...
/** Mount the existing product UI over an ACP-native Desktop object layer. */
export async function mountDesktopProduct(container: HTMLElement): Promise<() => void> {
  ...
  const unsubscribeFrames = window.dshDesktop.subscribe((frame) => {
    if (frame.type === 'session-update') sessions.accept(frame)
    if (frame.type === 'runtime-status' && frame.status === 'failed') {
