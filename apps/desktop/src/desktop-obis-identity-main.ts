...
function parseStored(value: unknown): StoredObisIdentity | undefined {
  ...
  const memberships = Array.isArray(row.memberships)
    ? row.memberships.filter(validMembership)
    : undefined
  return {
    ...
    ...(typeof row.installationId === 'string' ? { installationId: row.installationId } : {}),
    ...
    ...(
      row.user && typeof row.user.id === 'string' && typeof row.user.displayName === 'string'
        ? { user: row.user }
        : {}
    ),
    ...(validMembership(row.membership) ? { membership: row.membership } : {}),
...
function publicStatus(stored: StoredObisIdentity | undefined): DesktopObisIdentityStatus {
  return {
    ...
    authenticated: stored?.encryptedRefreshToken !== undefined
      && stored.user !== undefined
      && stored.membership !== undefined,
...
function encrypted(value: string): string {
  if (!safeStorage.isEncryptionAvailable()) {
    throw new Error('macOS secure storage is unavailable; OBIS credentials were not saved')
  }
...
function decrypted(value: string | undefined): string | undefined {
  ...
  if (!safeStorage.isEncryptionAvailable()) {
    throw new Error('macOS secure storage is unavailable; OBIS credentials cannot be read')
  }
...
function withoutSession(stored: StoredObisIdentity): StoredObisIdentity {
  return {
    ...
    ...(stored.installationId ? { installationId: stored.installationId } : {}),
  }
}
...
function channel(): 'canary' | 'stable' | 'enterprise-lts' {
  const value = process.env.OBIS_HARNESS_CHANNEL?.trim()
  return value === 'canary' || value === 'enterprise-lts' ? value : 'stable'
}

function installationBody(stored: StoredObisIdentity) {
  return {
    deviceId: stored.deviceId,
    deviceName: `${process.platform}-${process.arch}`,
    os: process.platform,
    architecture: process.arch,
    harnessVersion: app.getVersion(),
      ...
      const heartbeat = await requestJson<InstallationEnvelope>(
        ...
        `/v1/harness/installations/${encodeURIComponent(stored.installationId)}/heartbeat`,
        {
          ...
          body: {
            ...
            bridgeVersion: OHP_VERSION,
            protocolVersions: [OHP_VERSION],
            capabilities: installationBody(stored).capabilities,
          },
          accessToken,
          ohp: true,
        },
      )
      if (heartbeat.value.installation.status !== 'revoked') return stored
    } catch (error) {
      if (!(error instanceof ObisHttpError && error.status === 404)) throw error
    }
  }
...
async function authenticatedStored(): Promise<{ stored: StoredObisIdentity; accessToken: string }> {
  let stored = await ensureStored()
  if (!stored) throw new Error('Configure OBIS before using enterprise controls')
  let accessToken = decrypted(stored.encryptedAccessToken)
  const expiring = stored.expiresAt !== undefined && Date.parse(stored.expiresAt) <= Date.now() + 30_000
  if (!accessToken || expiring) {
    stored = await refreshStored(stored)
    accessToken = decrypted(stored.encryptedAccessToken)
  }
  if (!accessToken) throw new Error('Sign in to OBIS before using enterprise controls')
...
async function currentIdentity(): Promise<DesktopObisIdentityStatus> {
  ...
  const stillValid = accessToken !== undefined
...
function parseScopeRequest(value: unknown): DesktopEnterpriseScopeRequest {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error('Enterprise runtime scope request must be an object')
  }
  const row = value as Partial<DesktopEnterpriseScopeRequest>
  ...
  if (!projectId || !environmentId) {
    throw new Error('Enterprise runtime scope requires projectId and environmentId')
  }
...
function validatedScope(
  ...
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error('OBIS returned an invalid enterprise runtime scope')
  }
  const row = value as Partial<DesktopEnterpriseRuntimeScope>
  const scope: DesktopEnterpriseRuntimeScope = {
    ...
    installationId: typeof row.installationId === 'string' ? row.installationId : '',
    userId: typeof row.userId === 'string' ? row.userId : '',
  }
  if (
    scope.tenantId !== expected.tenantId
    || scope.projectId !== expected.projectId
    || scope.environmentId !== expected.environmentId
    || scope.installationId !== expected.installationId
    || scope.userId !== expected.userId
  ) {
    throw new Error('OBIS returned a runtime scope that does not match the authenticated desktop authority')
  }
  return scope
}

async function validateRuntimeScope(value: unknown): Promise<DesktopEnterpriseRuntimeScope> {
  const request = parseScopeRequest(value)
  const authenticated = await authenticatedStored()
  const stored = await ensureInstallation(authenticated.stored, authenticated.accessToken)
  const installationId = stored.installationId
  const userId = stored.user?.id
  if (!installationId || !userId) {
    throw new Error('Authenticated OBIS identity is missing Harness installation or user authority')
  }

  const response = await requestJson<unknown>(stored, '/v1/desktop/scope/validate', {
    method: 'POST',
    body: {
      tenantId: stored.tenantId,
  ...
  const scope = validatedScope(response.value, {
    tenantId: stored.tenantId,
    projectId: request.projectId,
    environmentId: request.environmentId,
    installationId,
  ...
  setEnterpriseRuntimeScope(scope)
  return scope
}
...
function installIdentityIpc(): void {
  ...
  ipcMain.handle('dsh:obis-identity-configure', async (event, value: unknown) => {
    requireTrusted(event)
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
      throw new Error('OBIS identity configuration must be an object')
    }
  ...
  ipcMain.handle('dsh:obis-context', async (event) => {
    requireTrusted(event)
    const { stored, accessToken } = await authenticatedStored()
    return (await requestJson<DesktopEnterpriseContext>(stored, '/v1/desktop/context', { accessToken })).value
  })

  ipcMain.handle('dsh:obis-switch-tenant', async (event, target: unknown) => {
    requireTrusted(event)
    if (typeof target !== 'string' || !target.trim()) throw new Error('Target tenant is required')
    const { stored, accessToken } = await authenticatedStored()
    if (target.trim() === stored.tenantId) return publicStatus(stored)
    const response = await requestJson<TokenPair>(stored, '/v1/auth/switch-tenant', {
      method: 'POST',
      body: { targetTenantId: target.trim() },
      accessToken,
    })
    ...
    setEnterpriseRuntimeScope(undefined)
    return publicStatus(await attachIdentity(nextAuthority, response.value))
  })

  ipcMain.handle('dsh:obis-workspace', async (event) => {
    requireTrusted(event)
    const { stored, accessToken } = await authenticatedStored()
    return (await requestJson<DesktopResolvedWorkspace>(stored, '/v1/desktop/workspace', { accessToken })).value
  })

  ipcMain.handle('dsh:obis-workspace-preferences', async (event, value: unknown) => {
    requireTrusted(event)
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
      throw new Error('Desktop preferences must be an object')
    }
    const { stored, accessToken } = await authenticatedStored()
    return (await requestJson<DesktopUserPreference>(stored, '/v1/desktop/preferences', {
      method: 'PUT',
      body: value,
      accessToken,
    })).value
  ...
  ipcMain.handle('dsh:obis-devices', async (event) => {
    requireTrusted(event)
    const { stored, accessToken } = await authenticatedStored()
    return (await requestJson<{ items: DesktopHarnessDevice[] }>(
      stored,
      '/v1/desktop/devices',
      { accessToken },
    )).value.items
  ...
  ipcMain.handle('dsh:obis-device-revoke', async (event, installationId: unknown) => {
    requireTrusted(event)
    if (typeof installationId !== 'string' || !installationId.trim()) {
      throw new Error('Installation id is required')
    }
    const { stored, accessToken } = await authenticatedStored()
    const devices = (await requestJson<{ items: DesktopHarnessDevice[] }>(
      stored,
      '/v1/desktop/devices',
      { accessToken },
    )).value.items
    const target = devices.find(item => item.id === installationId)
    await requestJson<unknown>(stored, `/v1/desktop/devices/${encodeURIComponent(installationId)}/revoke`, {
      method: 'POST',
      accessToken,
    })
    if (target?.deviceId === stored.deviceId) {
      setEnterpriseRuntimeScope(undefined)
      await writeStored(withoutSession(stored))
    }
  })

  ipcMain.handle('dsh:obis-workspace-definitions', async (event) => {
    requireTrusted(event)
    const { stored, accessToken } = await authenticatedStored()
    return (await requestJson<{ items: unknown[] }>(
      stored,
      '/v1/management/desktop/workspaces',
      { accessToken },
    )).value.items
  })

  ipcMain.handle('dsh:obis-workspace-definition-save', async (event, id: unknown, value: unknown) => {
    requireTrusted(event)
    if (typeof id !== 'string' || !id.trim() || typeof value !== 'object' || value === null || Array.isArray(value)) {
      throw new Error('Workspace id and definition are required')
    }
    const { stored, accessToken } = await authenticatedStored()
    return (await requestJson<unknown>(
      stored,
      `/v1/management/desktop/workspaces/${encodeURIComponent(id)}`,
      { method: 'PUT', body: value, accessToken },
    )).value
  })

  ipcMain.handle('dsh:enterprise-runtime-scope-validate', async (event, value: unknown) => {
    requireTrusted(event)
    return validateRuntimeScope(value)
  })
  ...
  ipcMain.handle('dsh:obis-identity-logout', async (event) => {
    ...
    let accessToken = decrypted(stored.encryptedAccessToken)
    const expired = stored.expiresAt !== undefined && Date.parse(stored.expiresAt) <= Date.now()
    if ((!accessToken || expired) && stored.encryptedRefreshToken) {
      try {
    ...
    if (accessToken) {
      try {
        await requestJson<unknown>(stored, '/v1/auth/logout', { method: 'POST', accessToken })
      } catch {
        // Local credentials are destroyed even if OBIS is offline.
      }
    }
