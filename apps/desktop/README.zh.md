...
- 技术验证 Renderer 可以创建和加载 Session、提交提示词、取消活动轮次、回答权限请求，并显示流式文本、默认展开的实时推理和工具 ACP 更新。配置的 OpenAI 兼容主模型会启用高强度推理。设置会在用户明确开启前保持 Computer Use 关闭；可用时使用本机 Chromium DevTools 端点，否则使用 macOS 辅助功能，截图和每次输入操作仍会请求批准。
